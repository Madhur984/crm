...
/* Nested display data (stored as JSON strings on Project) */
const PROJECTS = {
  p1: {
...
async function main() {
  ...
  const org = await prisma.organization.create({ data: { name: "Nordwerk Industrial GmbH" } });

  const passwordHash = await bcrypt.hash("recon1234", 10);
  await prisma.user.createMany({
    data: [
      { email: "demo@reconcore.app", passwordHash, name: "J. Weber", role: "Procurement Lead", orgId: org.id },
      { email: "alvarez@reconcore.app", passwordHash, name: "R. Alvarez", role: "Procurement Lead", orgId: org.id },
    ],
  });
  ...
  // Activity (preserve original order via `order`)
  ...
  console.log("Seed complete: 1 org, 2 users, 2 projects, documents & activity loaded.");
}
